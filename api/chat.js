export default async function handler(req, res) {
  ...
  try {
    ...
    const isEguraldia = ['eguraldia','tiempo','temperatura','euri','lluvia','elur','nieve','haize','viento','hotza','beroa','laino','lanbro','eguzkia','sol','zerua','eguraldi'].some(w => msg2.includes(w));
    ...
    if (isEguraldia) {
      ...
      try {
        const wr = await fetch('https://api.open-meteo.com/v1/forecast?latitude='+lat+'&longitude='+lon+'&current=temperature_2m,precipitation,windspeed_10m,weathercode&daily=temperature_2m_max,temperature_2m_min,precipitation_sum,weathercode&timezone=Europe/Madrid&forecast_days=3');
        ...
        ctx = 'EGURALDIA '+mk+': Orain '+c.temperature_2m+'C '+(wc[c.weathercode]||'')+' euria:'+c.precipitation+'mm haizea:'+c.windspeed_10m+'kmh. Bihar max'+d.temperature_2m_max[1]+'C min'+d.temperature_2m_min[1]+'C. Etzi max'+d.temperature_2m_max[2]+'C min'+d.temperature_2m_min[2]+'C';
