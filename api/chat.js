export default async function handler(req, res) {
  ...
  try {
    ...
    const ITURRIAK = {
      ...
      municipios: {
        ...
        'zarautz':'https://zarautzguka.eus/zarautz/','zumaia':'https://baleike.eus','azpeitia':'https://uztarria.eus','azkoitia':'https://maxixatzen.eus','getaria':'https://guka.eus/getaria/','orio':'https://karkara.eus','aia':'https://karkara.eus',
    ...
    const isEguraldia = ['eguraldia','eguraldi','euraldia','euraldi','tiempo','temperatura','euri','lluvia','elur','nieve','haize','viento','hotza','beroa','laino','lanbro','eguzkia','sol','zerua','eguraldi','guraldi','gipuzkoa'].some(w => msg2.includes(w));
    ...
    const isUD = ['baimena','tramite','izapidea','dirulaguntza','errolda','eskaera','zerbitzuak','laguntza','ayuda','subvencion','gestion','sala','pisos tutelados','tutelado','vivienda','etxebizitza','residencia','zaharren egoitza','dependencia','mendekotasun','teleasistencia','telelaguntza','rgi','padron','empadron','udaletxe','ayuntamiento','udala'].some(w => msg2.includes(w));
    const udalUrls = {
  ...
  'gazteluko':{egoitza:'https://udal.egoitza.gipuzkoa.eus/WAS/AYTO/USCServicioCiudadanoVer15WEB/home.do?ayto=088&hizkuntza=EU'}
    ...
    if (isEguraldia) {
      ...
      if (mk === 'gipuzkoa') { try {
        const wr3 = await fetch('https://api.open-meteo.com/v1/forecast?latitude=43.321,43.065,43.048&longitude=-1.984,-2.490,-2.172&current=temperature_2m&timezone=Europe/Madrid');
        const wa3 = await wr3.json(); const r3 = (n) => Math.round(n); const p3 = Array.isArray(wa3) ? wa3 : [wa3];
        ctx = 'EGURALDIA Gipuzkoa hiru paisaietan: Kostaldean (Donostia) orain '+r3(p3[0].current.temperature_2m)+' gradu. Debagoienan (Arrasate) '+r3(p3[1].current.temperature_2m)+' gradu. Goierrin (Ordizia) '+r3(p3[2].current.temperature_2m)+' gradu.';
      } catch(e){} } else
      try {
      ...
      const munNO = Object.keys(ITURRIAK.municipios).find(k => msg2.includes(k));
      const nurl = munNO ? ITURRIAK.municipios[munNO] : (noticiaUrls[comarca] || null);
      ...
      if (uMk && udalUrls[uMk].web) { searchQuery = message + ' 2026 site:' + udalUrls[uMk].web.replace('https://','').replace('http://','').split('/')[0]; } else if (uMk) { searchQuery = message + ' ' + uMk + ' udala gipuzkoa.eus'; } else { searchQuery = message + ' 2026 site:elgoibar.eus OR site:eibar.eus OR site:gipuzkoa.eus'; }
    } else if (isAO) {
