export default async function handler(req, res) {
  ...
  try {
    ...
    const isES = /\b(el|la|los|las|de|en|que|es|un|una|para|por|con|del|al|hay|si|hoy|resultado|partido|futbol|noticias|final|ayer|farmacia|guardia|agenda|fiestas|horario|tren|cuantos|personas|mayores|quien|hubo|fue|habitantes|poblacion|datos)\b/i.test(msg);
    ...
    const agendaUrls = {db:'https://lea-artibaietamutriku.hitza.eus/agenda/gaur/',dg:'https://goiena.eus/',go:'https://goiena.eus/',tl:'https://tolosaldea.hitza.eus/',oa:'https://oarsoaldea.hitza.eus/',ur:'https://urola.hitza.eus/',bd:'https://bidasoa.hitza.eus/',uk:'https://lea-artibaietamutriku.hitza.eus/agenda/gaur/'};
    const noticiaUrls = {db:'https://lea-artibaietamutriku.hitza.eus/',dg:'https://goiena.eus/',go:'https://goiena.eus/',tl:'https://tolosaldea.hitza.eus/',oa:'https://oarsoaldea.hitza.eus/',ur:'https://urola.hitza.eus/',bd:'https://bidasoa.hitza.eus/',uk:'https://lea-artibaietamutriku.hitza.eus/'};
    ...
    const isAO = ['agenda','ekitaldi','fiestas','jaiak','kontzertua','concierto','exposicion','erakusketa'].some(w => msg2.includes(w));
    ...
    if (isEguraldia) {
      ...
      try {
        ...
        const h = wd.hourly; const now = new Date(); const todayStr = now.toISOString().slice(0,10); const h12 = h.time.findIndex(t => t === todayStr+'T12:00'); const h18 = h.time.findIndex(t => t === todayStr+'T18:00'); const t12 = h12>=0 ? h.temperature_2m[h12]+'C '+(wc[h.weathercode[h12]]||'') : ''; const t18 = h18>=0 ? h.temperature_2m[h18]+'C '+(wc[h.weathercode[h18]]||'') : ''; const days=['igandea','astelehena','asteartea','asteazkena','osteguna','ostirala','larunbata']; const months=['urt','ots','mar','api','mai','eka','uzt','abu','ira','urr','aza','abe']; const d1=new Date(d.time[1]+'T12:00'); const d2=new Date(d.time[2]+'T12:00'); const bihar_str=days[d1.getDay()]+' '+d1.getDate()+' '+months[d1.getMonth()]; const etzi_str=days[d2.getDay()]+' '+d2.getDate()+' '+months[d2.getMonth()]; ctx = 'EGURALDIA '+mk+': Orain '+c.temperature_2m+'C '+(wc[c.weathercode]||'')+' haizea:'+c.windspeed_10m+'kmh. Eguerdian(12h):'+t12+'. Arratsaldean(18h):'+t18+'. Bihar('+bihar_str+') max'+d.temperature_2m_max[1]+'C min'+d.temperature_2m_min[1]+'C. Etzi('+etzi_str+') max'+d.temperature_2m_max[2]+'C min'+d.temperature_2m_min[2]+'C';
